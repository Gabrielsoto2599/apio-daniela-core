...
memorias_quibor = [
    {
        "tag": "la_noche_que_nos_conocimos",
...
# 🚀 PUENTE DE CONEXIÓN CON EL ORM DE DJANGO
def migrar_memorias_al_orm():
    """
    Función semilla que lee esta lista de Python e inyecta los datos de forma
    persistente en el modelo de base de datos de Django para el RAG.
    """
    try:
        # Importamos el modelo de tu app 'cerebro' dentro de la función para evitar problemas de carga
        from cerebro.models import MemoriaSubconsciente
        
        print("⏳ Insertando memorias núcleo de Quíbor en el ORM...")
        registros_creados = 0
        
        for mem in memorias_quibor:
            # Evitamos duplicados buscando por la etiqueta única (tag)
            obj, created = MemoriaSubconsciente.objects.get_or_create(
                contenido_texto=f"[{mem['evento'].upper()}] - {mem['relato']}",
                defaults={
                    'contexto': 'NOVIA_POSESIVA'
                }
            )
            if created:
                registros_creados += 1
                
        print(f"✅ Migración exitosa: {registros_creados} nuevas memorias núcleo listas en el ORM.")
